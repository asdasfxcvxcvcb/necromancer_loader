...
#ifdef _WIN64
	// WOW64 handler is optional - only needed for injecting into 32-bit processes
	// Don't fail if SM86.exe is missing, just log a warning
	if (import_handler_wow64_ret.wait_for(std::chrono::milliseconds(0)) != std::future_status::ready)
	{
		// WOW64 still loading, but native is ready - that's OK for x64 injection
		LOG(0, "Import handler (wow64) still loading, but native is ready\n");
	}
	else
	{
		...
		if (imp_ret != INJ_ERR_SUCCESS)
		{
			// Log but don't fail - WOW64 is optional for x64-only injection
			LOG(0, "Import handler (wow64) failed: %08X (non-fatal for x64 injection)\n", imp_ret);
		}
	}
#endif
